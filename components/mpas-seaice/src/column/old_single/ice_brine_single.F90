...
!
      ...
      use ice_colpkg_tracers_single, only: ntrcr, nt_qice, nt_sice, nt_bgc_S 
      ...
      real (kind=real_kind), parameter :: &   
         maxhbr  = 1.25_real_kind  , & ! brine overflows if hbr > maxhbr*hin
         viscos  = 2.1e-6_real_kind, & ! kinematic viscosity (m^2/s) 
         ! Brine salinity as a cubic function of temperature
         a1      = -21.4_real_kind , & ! (psu/C)  
         a2      = -0.886_real_kind, & ! (psu/C^2)
         a3      = -0.012_real_kind, & ! (psu/C^3)
         ! Brine density as a quadratic of brine salinity
         b1      = 1000.0_real_kind, & ! (kg/m^3)  
         b2      = 0.8_real_kind       ! (kg/m^3/ppt)
...
! average ice permeability and the surface ice porosity
      ...
      use ice_therm_mushy_single, only: permeability
      use ice_mushy_physics_single, only: temperature_mush, liquid_fraction
      use ice_colpkg_shared_single, only: l_sk, min_salin
      ...
      real (kind=real_kind), dimension (ntrcr+2) :: & ! nblyr+2)
...
!=======================================================================
      ...
      use ice_colpkg_shared_single, only: rhosi
...
!                        / (brine_sal(1)*brine_rho(1))*phi_snow)
      ...
      if (k_min > c0) then
         ...
         kperm = real(nblyr,kind=real_kind)/ktemp
      endif
...
! 
      ...
      use ice_therm_shared_single, only: calculate_Tin_from_qin
      use ice_colpkg_tracers_single, only: nt_fbri, nt_Tsfc
      use ice_colpkg_shared_single, only: min_salin, rhosi, salt_loss
...
! for gravity drainage parameterization
      ...
      real (kind=real_kind), dimension (nblyr + 1) :: &  !on the zbgc vertical grid
     ...
     do k = 1, nblyr+1   !i_grid values
        ...
        drho(k) = max(rho_b(k) - rho_2b(k),max(c0,c2*(rho_a(k)-rho_2a(k)), &
              c2*(brine_rho(k)-brine_rho(k+1))/real(nblyr,kind=real_kind)))
     enddo
