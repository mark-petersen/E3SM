...
! authors: William H. Lipscomb and Elizabeth C. Hunke, LANL
      ...
      real (kind=real_kind), dimension (ncat), intent(inout) :: &
         aicen , & ! concentration of ice
...
! authors: William H. Lipscomb and Elizabeth C. Hunke, LANL
      ...
      real (kind=real_kind), dimension(ntrcr,ncat) :: &
         atrcrn            ! aicen*trcrn   
...
! author: William H. Lipscomb, LANL
      ...
      real (kind=real_kind), dimension (nsum), &
         intent(in) :: &
         xin              ! input field
...
! author: William H. Lipscomb, LANL
      ...
      real (kind=real_kind), dimension (n_aero) :: &
...
! author: William H. Lipscomb, LANL
      ...
      use ice_colpkg_tracers_single, only: nt_Tsfc, nt_qice, nt_qsno, nt_aero, &
                             nt_apnd, nt_hpnd, nt_fbri, tr_brine, nt_bgc_S, &
                             bio_index
      use ice_colpkg_shared_single, only:  solve_zsal, skl_bgc, z_tracers, min_salin, &
                             rhosi
      use ice_constants_colpkg, only: sk_l
      use ice_zbgc_shared_single, only: zap_small_bgc
      ...
      real (kind=real_kind) :: xtmp      ! temporary variables
      real (kind=real_kind) , dimension (1):: trcr_skl    
      real (kind=real_kind) , dimension (nblyr+1):: bvol     
      ...
      !-----------------------------------------------------------------
         ...
         elseif (z_tracers .and. nbtrcr > 0) then
               blevels = nblyr + 1
               bvol(:) = vicen(n)/real(nblyr,kind=real_kind)*trcrn(nt_fbri,n)
               bvol(1) = p5*bvol(1)
      ...
      !-----------------------------------------------------------------
            ...
            do k = 1, nilyr
               xtmp = trcrn(nt_qice+k-1,n) / dt &
                    * vicen(n)/real(nilyr,kind=real_kind) ! < 0
               dfhocn = dfhocn + xtmp
      ...
      !----------------------------------------------------------------- 
            ...
            do k = 1, nilyr 
               xtmp = trcrn(nt_qice+k-1,n) &
                    * vicen(n)/real(nilyr,kind=real_kind) &
                    * (aice-c1)/aice / dt ! < 0 
               dfhocn = dfhocn + xtmp 
            enddo                  ! k 
      ...
      !----------------------------------------------------------------- 
            ...
            do k = 1, nslyr 
               xtmp = trcrn(nt_qsno+k-1,n) &
                    * vsnon(n)/real(nslyr,kind=real_kind) &
                    * (aice-c1)/aice / dt ! < 0 
               dfhocn = dfhocn + xtmp 
            enddo                  ! k
      ...
      !-----------------------------------------------------------------
            ...
            do k = 1,nblyr
               xtmp = rhosi*trcrn(nt_fbri,n)*vicen(n)*p001&
                    /real(nblyr,kind=real_kind)*trcrn(nt_bgc_S+k-1,n) &
                    * (aice-c1)/aice /dt
               dfzsal = dfzsal + xtmp
            enddo
...
!=======================================================================
      ...
      use ice_colpkg_tracers_single, only: nt_qsno, nt_aero, bio_index
      use ice_colpkg_shared_single, only: hs_ssl, z_tracers
     ...
     real (kind=real_kind), dimension (:), intent(inout) :: &
      ...
      real (kind=real_kind) :: xtmp, dvssl, dvint
      ...
      if (z_tracers) then
            dvssl  = min(p5*vsnon/real(nslyr,kind=real_kind), hs_ssl*aicen)   !snow surface layer
      ...
      do k = 1, nslyr 
         xtmp = trcrn(nt_qsno+k-1) / dt &
              * vsnon/real(nslyr,kind=real_kind) ! < 0
         dfhocn = dfhocn + xtmp
...
!=======================================================================
      ...
      use ice_colpkg_tracers_single, only: nt_qsno 
      use ice_therm_shared_single, only: Tmin
      ...
      rnslyr = real(nslyr,kind=real_kind)
...
! over k, making the test more stringent.
      ...
      use ice_colpkg_tracers_single, only: nt_qice, nt_qsno
