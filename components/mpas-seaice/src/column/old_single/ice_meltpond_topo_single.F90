...
!=======================================================================
      ...
      real (kind=real_kind), parameter :: &
         ...
         min_volp = 1.e-4_real_kind  ! minimum pond volume (m)
      ...
      do n = 1, ncat
         ...
         if (aicen(n) < puny)  then
            ...
            if (n < ncat) reduced_aicen(n) = aicen(n) &
                * max(0.2_real_kind,(-0.024_real_kind*hicen(n) + 0.832_real_kind))
...
!=======================================================================
  ...
  subroutine calc_hpond(ncat, aicen, asnon, hsnon, &
    ...
    real (kind=real_kind), dimension(0:ncat+1) :: &
         hitl, &
         aicetl
...
! determine the liquid fraction of brine in the ice and the permeability
      ...
      if (maxval(Tin) <= -c2) then
         ...
         do k = 1,nilyr
            Sbr = - 1.2_real_kind                 &
                  -21.8_real_kind     * Tin(k)    &
                  - 0.919_real_kind   * Tin(k)**2 &
                  - 0.01878_real_kind * Tin(k)**3
            if (heat_capacity) then
         ...
         do k = 1,nilyr
            Sbr = -17.6_real_kind    * Tin(k)    &
                  - 0.389_real_kind  * Tin(k)**2 &
                  - 0.00362_real_kind* Tin(k)**3
            if (Sbr == c0) then
               l_stop = .true.
      ...
      perm = 3.0e-08_real_kind * (minval(phi))**3
