...
!  and vertical cycling
      ...
      use ice_colpkg_shared_single, only: hi_ssl, hs_ssl, max_aero
      use ice_colpkg_tracers_single, only: nt_aero 
      ...
      real (kind=real_kind), dimension(max_aero) :: &
      ...
      real (kind=real_kind), dimension(n_aero,2) :: &
      ...
      data kscav   / .03_real_kind, .20_real_kind, .02_real_kind, &
                     .02_real_kind, .01_real_kind, .01_real_kind / 
      data kscavsi / .03_real_kind, .20_real_kind, .02_real_kind, &
                     .02_real_kind, .01_real_kind, .01_real_kind / 
    ...
    !-------------------------------------------------------------------
      ...
      hslyr_old = hs_old/real(nslyr,kind=real_kind)
      hilyr_old = hi_old/real(nilyr,kind=real_kind)
    ...
    !-------------------------------------------------------------------
      ...
      hslyr      = hs/real(nslyr,kind=real_kind)
...
!  and vertical cycling : after update_aerosol
      ...
      use ice_colpkg_shared_single, only: hi_ssl, hs_ssl, hs_ssl_min
      use ice_constants_colpkg_single, only: c0, rhos, rhoi, hs_min, puny, &
                         c2, c1, p5
      use ice_zbgc_shared_single, only: kscavz
      ...
      real (kind=real_kind),dimension(nbtrcr), intent(out) :: &
      ...
      real (kind=real_kind), dimension(nbtrcr,2) :: &
         aerosno,  & ! kg/m^2
    ...
    !-------------------------------------------------------------------
         ...
         hilyr = hi/real(nblyr,kind=real_kind)
         hslyr = hs/real(nslyr,kind=real_kind)
         dzssl_new  = min(hslyr/c2, hs_ssl)
         dhs_melts  = -melts
