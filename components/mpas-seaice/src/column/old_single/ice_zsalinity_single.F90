...
!
      ...
      real (kind=real_kind), parameter :: & 
         max_salin = 200.0_real_kind, & !(ppt) maximum bulk salinity
         lapidus_g = 0.3_real_kind  , & ! constant for artificial 
                                       ! viscosity/diffusion during growth
         lapidus_m = 0.007_real_kind    ! constant for artificial diffusion during melt
...
!
      ...
      real (kind=real_kind), dimension (2), intent(in) :: &
         S_bot
      ...
      if (hbri_old > thinS .AND. hbrin > thinS .and. &
         ...
         lapidus = lapidus_g/real(nblyr,kind=real_kind)**2
         ivel(1) = dht/hbri_old
         U_s (1) = ivel(1)/dt*dts 
         Ui_s(1) = U_s(1) 
         Ci_s(1) = c0
      ...
      !-----------------------------------------------------------------
            ...
            do k = 2, nblyr+1  
               ...
               lapidus_diff(k-1) =    lapidus/& ! lapidus/real(nblyr,kind=real_kind)**2/&
                  (igrid(k)-igrid(k-1))* &
...
! 
      ...
      real (kind=real_kind) :: hin_old, hin_next, dhtmp !, dh

      dhtmp = c1-dh/hbri_old
      hin_next = hbri_old +  real(mint,kind=real_kind)*dh
      hin_old  = hbri_old + (real(mint,kind=real_kind)-c1)*dh
...
!  
     ...
     real (kind=real_kind), parameter :: &
         accuracy = 1.0e-7_real_kind ! g/kg/m^2/s difference between boundary fluxes 

     character(len=char_len_long) :: &
         warning ! warning message
     
         dh = (hbrin-hbri_old)/real(mmax,kind=real_kind)

         flux_tot = (fluxb + fluxg + fluxm + fluxcorr + Ssum_corr)*&
                    (hbri_old + (real(mint,kind=real_kind))*dh)/dt
         dsum_flux =(Ssum_new*(hbri_old + (real(mint,kind=real_kind))*dh) - &
                     Ssum_old*(hbri_old + (real(mint,kind=real_kind)-c1)* &
                     dh) )/dt
...
! integration while "avg"  is normalized by the ice thickness
      ...
      real (kind=real_kind), dimension (nblyr), intent(in) :: &
         trcrn_S       ! input field
      ...
      do k = 1, nblyr
         zsal_totn = zsal_totn &
                   + rhosi * trcrn_S(k) &
                           * fbri  &
                           * vicenS/real(nblyr,kind=real_kind)
      enddo ! k
