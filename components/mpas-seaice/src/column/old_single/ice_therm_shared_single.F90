...
! authors: Elizabeth C. Hunke, LANL
      ...
      real (kind=real_kind), parameter, public :: &
         ferrmax = 5.0e-2_real_kind    ! max allowed energy flux error (W m-2)
                                      ! recommend ferrmax < 0.01 W m-2

      real (kind=real_kind), parameter, public :: &
         Tmin = -100.0_real_kind ! min allowed internal temperature (deg C)

      logical (kind=log_kind), public :: &
         l_brine         ! if true, treat brine pocket effects

      real (kind=real_kind), parameter, public :: &
         hfrazilmin = 0.05_real_kind ! min thickness of new frazil ice (m)
