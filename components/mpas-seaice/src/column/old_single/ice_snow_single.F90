...
!         Nicole Jeffery, LANL
      ...
      real (kind=real_kind), parameter, public :: &
         S_r  = 0.033_real_kind, & ! irreducible saturation (Anderson 1976)
         S_wet= 0.422_real_kind  ! (um^3/s) wet metamorphism parameters
...
! Compute effective density of snow layers from ice, liquid water mass
      ...
      !-----------------------------------------------------------------
         ...
         enddo
         rhos_eff = rhos_eff/(vsno*real(nslyr,kind=real_kind))
         rhos_cmp = rhos_cmp/(vsno*real(nslyr,kind=real_kind))
...
! thickness does not
      ...
      use ice_therm_vertical_single, only: adjust_enthalpy
      ...
      real (kind=real_kind), parameter :: &
         ...
         gamma   = 1.e-5_real_kind    ! tuning coefficient
      ...
      real (kind=real_kind), dimension (nslyr+1) :: &
...
!echmod      flost = c0
      ...
      do n = 1, ncat
         ...
         Vseas = (rhos_cmpn(1,n) - 44.6_real_kind)/174.0_real_kind ! use top layer
         Vseas = max(Vseas, c0)
      ...
      !-----------------------------------------------------------------
               ...
               if (nslyr > 1) then
                  ...
                  dzs(:) = hsn(n) / real(nslyr,kind=real_kind) ! old layer thickness
                  do k = 1, klyr(n)
                     dzs(k) = dzs(k) + dhsn / klyr(n)         ! old layer thickness (updated)
                  enddo
                  hsn_new(n) = hsn_new(n) + dhsn
                  hslyr  = hsn_new(n) / real(nslyr,kind=real_kind) ! new layer thickness
      ...
      do n = 1, ncat
         if (vsn(n) > puny) then
            ...
            hslyr = hsn_new(n) / real(nslyr,kind=real_kind)
            do k = 1, nslyr
               work = hslab - hslyr * real(k-1,kind=real_kind)
...
!  Snow grain metamorphism driver
      ...
      real (kind=real_kind), dimension(ncat), intent(in) :: &
         zTin        , & ! surface ice temperature (oC)
         Tsfc        , & ! surface temperature (oC)
         hin         , & ! ice thickness (m)
         hsn             ! snow thickness (m)

      real (kind=real_kind), dimension(nslyr,ncat), intent(in) :: &
         zqsn            ! enthalpy of snow (J m-3)

      real (kind=real_kind), dimension(nslyr,ncat), intent(inout) :: &
         rsnw

      real (kind=real_kind), dimension(nslyr,ncat), &
         intent(inout) :: &
...
!  Snow grain metamorphism
      ...
      use ice_colpkg_shared_single, only: idx_T_min, idx_Tgrd_min, idx_rhos_min
...
! calculate density based on liquid and ice content of snow
         ...
         dzs = hsn/real(nslyr,kind=real_kind)
         dzi = hin/real(nilyr,kind=real_kind)
         ...
         do k = 1, nslyr
          ...
          T_idx    = nint(abs(zTsn(k)+ Tffresh - 223.0_real_kind) / 5.0_real_kind, kind=int_kind)
          Tgrd_idx = nint(zdTdz(k) / 10.0_real_kind, kind=int_kind)
          !rhos_idx = nint(zrhos(k)-50.0_real_kind) / 50.0_real_kind, kind=int_kind)   ! variable density
          rhos_idx = nint((rhos-50.0_real_kind) / 50.0_real_kind, kind=int_kind)        ! fixed density
...
!  Conversions between ice mass, liquid water mass in snow
      ...
      if (hsn > puny) then
        dlin(:) = c0
        dlout(:) = c0
        hslyr    = hsn / real(nslyr,kind=real_kind)
        meltsliq = c0
        do k = 1,nslyr
