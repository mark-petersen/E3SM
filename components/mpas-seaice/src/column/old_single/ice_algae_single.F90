...
!=======================================================================
      ...
      use ice_aerosol_single, only: update_snow_bgc
      use ice_constants_colpkg, only: c0, c1, puny, p5
      use ice_zbgc_single, only: merge_bgc_fluxes
      ...
      real (kind=real_kind), dimension (nblyr+2), intent(inout) :: &
      ...
      real (kind=real_kind), dimension(ntrcr), &
         intent(inout) :: &
         trcrn
      ...
      real (kind=real_kind), dimension (nbtrcr), intent(in) :: &
      ...
      real (kind=real_kind), dimension (nblyr+1,n_algae) :: &
      ...
      real (kind=real_kind),dimension(nbtrcr) :: &
...
!=======================================================================
      ...
      use ice_zbgc_single, only: merge_bgc_fluxes_skl
      use ice_colpkg_tracers_single, only: nt_bgc_N
      ...
      real (kind=real_kind), dimension(ntrcr), intent(inout) :: &
         trcrn      ! bulk concentration per m^3

      real (kind=real_kind), dimension (nbtrcr), intent(inout) :: &
      ...
      real (kind=real_kind), dimension (n_algae) :: &
         upNOn      , & ! algal nitrate uptake rate  (mmol/m^3/s)
         upNHn      , & ! algal ammonium uptake rate (mmol/m^3/s)
         grow_alg       ! algal growth rate          (mmol/m^3/s)
...
! 
      ...
      use ice_colpkg_tracers_single, only: nt_bgc_N,  ntrcr, bio_index 
      use ice_colpkg_shared_single, only: dEdd_algae, bgc_flux_type, R_chl2N
      ...
      real (kind=real_kind), parameter :: &
         PVc = 1.e-6_real_kind           , & ! type 'constant' piston velocity for interface (m/s) 
         ...
         growth_max = 1.85e-10_real_kind , & ! PVt function reaches maximum here.  (m/s)
         MJ1 = 9.667e-9_real_kind        , & ! (m/s) coefficients in Jin2008
         MJ2 = 38.8_real_kind            , & ! (1) from:4.49e-4_real_kind*secday   
         MJ3 = 1.04e7_real_kind          , & ! 1/(m/s) from: 1.39e-3_real_kind*secday^2  
         PV_frac_max = 0.9_real_kind         ! Maximum Piston velocity is 90% of skeletal layer/dt
...
!
      ...
      use ice_colpkg_tracers_single, only: nt_fbri, nt_zbgc_frac, &
      ...
      use ice_colpkg_shared_single, only: hi_ssl, dEdd_algae, solve_zbgc, &
      ...
      real (kind=real_kind), dimension(nblyr+1,nbtrcr):: &
      ...
      real (kind=real_kind), dimension(nblyr+1):: &  ! temporary variables for
         Diff         , & ! diffusivity
      ...
      real (kind=real_kind), parameter :: &
         accuracy = 1.0e-14_real_kind, &
         r_c  = 3.0e3_real_kind     , & ! ice crystal radius (um)
         r_bac= 4.7_real_kind    , & ! diatom large radius (um)
         r_alg= 10.0_real_kind    , & ! diatom small radius (um)
         Nquota_A = 0.88_real_kind, & ! slope in Nitrogen quota to cell volume fit
                                     ! (Lomas et al. 2019, Edwards et al. 2012)
         Nquota_I = 0.0408_real_kind, & ! Intercept in N quota to cell volume fit
         f_s = p1, & ! fracton of sites available for saturation
         f_a = 0.3_real_kind, & !c1 , &  ! fraction of collector available for attachment
  ...
  !-----------------------------------
      ...
      S_col   = 4.0_real_kind*pi*r_c**2
      P_b     = pi*r_bac**2    !*10-6 for colloids
      V_c     = 4.0_real_kind*pi*r_c**3/3.0_real_kind  !*(1.0e-6_real_kind)**3  (m^3) sphere
      V_alg   = pi/6.0_real_kind*r_bac*r_alg**2       ! prolate spheroid (*10-9 for colloids)
      Sat_conc= f_s*f_a*f_v*(c1-phi_max)/V_c*S_col/P_b*(V_alg)**Nquota_A*Nquota_I * 1.0e9_real_kind
...
! allow release of Nitrate/silicate, but not adsorption *
            ...
            elseif (tr_bgc_N .and. mm .eq. nlt_bgc_N(1)) then
               if (meltb > algal_vel*dt .or. aicen < 0.001_real_kind) then
      ...
      do m = 1,nbtrcr
         do k = 1,nblyr+1                  ! back to bulk quantity
            ...
            elseif (abs(bio_tmp) < puny) then
               flux_bio(m) = flux_bio(m) + bio_tmp*dz(k)*hbri_old/dt
               bio_tmp = c0
            elseif (bio_tmp > 1.0e8_real_kind) then
...
!          Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared_single, only: max_algae, max_DON, max_DOC, R_C2N, R_chl2N, &
                                   T_max, fsal      , fr_resp          , &
      ...
      use ice_zbgc_shared_single, only:   chlabs, alpha2max_low, beta2max, mu_max, &
                                   grow_Tdep, fr_graze, mort_pre, mort_Tdep, &
                                   k_exude, K_Nit, K_Am, K_Sil, K_Fe, &
                                   f_don, kn_bac, f_don_Am, &
                                   f_doc, f_exude, k_bac

      use ice_colpkg_tracers_single, only: tr_brine, nt_fbri, &
      ...
      real (kind=real_kind),  parameter, dimension(max_algae) :: &
         alpha2max_high  = (/ 0.25_real_kind, 0.25_real_kind, 0.25_real_kind/) ! light limitation (1/(W/m^2))
      real (kind=real_kind), parameter, dimension(max_algae) :: &
         graze_exponent = (/ 0.333_real_kind, c1, c1/) ! Implicit grazing exponent (Dunneet al. 2005)

      real (kind=real_kind), parameter :: &
         graze_conc = 1.36_real_kind ! (mmol N/m^3) converted from Dunne et al 2005
...
!  source terms underscore s, removal underscore r
      ...
      real (kind=real_kind), dimension(n_don) :: &
         DON_r      , &  ! net DON removal (mmol/m^3)
         DON_s           ! net DON sources (mmol/m^3)

      real (kind=real_kind), dimension(n_fed) :: &
      ...
      real (kind=real_kind), dimension(n_fep) :: &
...
!                      + f_exude(n)*exude_C
      ...
      !-----------------------------------------------------------------------
       ...
       if (tr_bgc_C) then
       if (abs(dC) > maxval(abs(reactb(:)))*1.0e-13_real_kind .or. &
          abs(dN) > maxval(abs(reactb(:)))*1.0e-13_real_kind) then
            conserve_C = .false.
...
!
      subroutine compute_FCT_matrix &
       ...
       use ice_colpkg_shared_single, only: grid_o
      ...
      real (kind=real_kind), intent(inout) :: &   ! positive into ice
      ...
      real (kind=real_kind), dimension (nblyr) :: &
         kvector1, kvectorn1
...
!
      ...
      real (kind=real_kind), dimension (nmat), &
           intent(in) :: &
         sbdiag      , & ! sub-diagonal matrix elements
         diag        , & ! diagonal matrix elements
         spdiag      , & ! super-diagonal matrix elements
...
! authors     Nicole Jeffery, LANL
     ...
     !-------------------------------------
         ...
         accuracy = 1.0e-14_real_kind*max(c1, C_init_tot, C_new_tot)
...
! author: Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared_single, only: hs_ssl
      ...
      real (kind=real_kind), dimension(nblyr+3), intent(in) :: &
         xin              ! input field
      ...
      hslyr      = hsnow/real(nslyr,kind=real_kind)
...
! author: Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared_single, only: hs_ssl, R_C2N, R_C2N_DON
      use ice_constants_colpkg, only: p5, c1, c0
      use ice_colpkg_tracers_single, only: tr_bgc_N, tr_bgc_C, tr_bgc_hum,   &
...
! author: Nicole Jeffery, LANL
      ...
      use ice_colpkg_shared_single, only: R_C2N, R_C2N_DON
