...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_itd_single, only: aggregate_area, shift_ice, & 
                         column_sum, column_conservation_check
      use ice_colpkg_tracers_single, only: nt_qice, nt_qsno, nt_fbri, nt_sice, &
...
!
      ...
      real(kind=real_kind), dimension(nilyr) :: trc2 ! updated tracer temporary
...
!
      ...
      use ice_colpkg_tracers_single, only: nt_qice, nt_qsno, nt_aero, tr_aero, &
                             tr_pond_topo, nt_apnd, nt_hpnd, bio_index
      use ice_colpkg_shared_single, only: z_tracers , hs_ssl, solve_zsal
      use ice_zbgc_single, only: lateral_melt_bgc
      ...
      !-----------------------------------------------------------------
            ...
            if (z_tracers) then   ! snow tracers
               dvssl  = min(p5*vsnon(n)/real(nslyr,kind=real_kind), hs_ssl*aicen(n))       !snow surface layer
...
!
      ...
      use ice_itd_single, only: column_sum, &
                         column_conservation_check 
      use ice_colpkg_tracers_single, only: nt_Tsfc, nt_iage, nt_FY, nt_sice, nt_qice, &
                             nt_alvl, nt_vlvl, nt_aero, nt_apnd, &
                             tr_pond_cesm, tr_pond_lvl, tr_pond_topo, &
                             tr_iage, tr_FY, tr_lvl, tr_aero, tr_brine
      use ice_colpkg_shared_single, only: solve_zsal, skl_bgc, initbio_frac, salt_loss, rhosi
      use ice_mushy_physics_single, only: liquidus_temperature_mush, enthalpy_mush
      use ice_therm_shared_single, only: hfrazilmin
      use ice_zbgc_single, only: add_new_ice_bgc
      use ice_zbgc_shared_single, only: bgc_tracer_type
     ...
     ! character (char_len), intent(out) :: stop_label
      ...
      rnilyr = real(nilyr,kind=real_kind)

      if (ncat > 1) then
         hi0max = hin_max(1)*0.9_real_kind  ! not too close to boundary
      else
