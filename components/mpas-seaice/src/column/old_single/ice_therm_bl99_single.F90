...
! 2012: Split from ice_therm_vertical.F90
      ...
      use ice_colpkg_shared_single, only: conduct, calc_Tsfc, solve_zsal
      use ice_therm_shared_single, only: ferrmax, l_brine, hfrazilmin
      use ice_warning_singles, only: add_warning
      ...
      real (kind=real_kind), parameter :: &
         betak   = 0.13_real_kind, & ! constant in formula for k (W m-1 ppt-1)
         kimin   = 0.10_real_kind    ! min conductivity of saline ice (W m-1 deg-1)
...
!         C. M. Bitz, UW
      ...
      use ice_therm_shared_single, only: surface_heat_flux, dsurface_heat_flux_dTsf
     ...
     ! local variables
      ...
      real (kind=real_kind), parameter :: &
         Tsf_errmax = 5.e-4_real_kind ! max allowed error in Tsf
      ...
      if (hslyr > hs_min/real(nslyr,kind=real_kind)) &
           l_snow = .true.
...
!                             * (cp_ocn - cp_ice + Lfresh/zTin(k))
      ...
      !-----------------------------------------------------------------
            ...
            if (ferr > 0.9_real_kind*ferrmax) then         ! condition (5)
...
!         C. M. Bitz, UW
      ...
      if (conduct == 'MU71') then
         ...
         do k = 1, nilyr
            kilyr(k) = (2.11_real_kind - 0.011_real_kind*zTin(k) &
                     + 0.09_real_kind*zSin(k)/min(-puny,zTin(k))) &
                     * rhoi / 917._real_kind
            kilyr(k) = max (kilyr(k), kimin)
         enddo                     ! nilyr
      endif ! conductivity
