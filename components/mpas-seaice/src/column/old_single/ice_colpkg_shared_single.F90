...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), parameter, public :: &
         saltmax = 3.2_real_kind,   & ! max salinity at ice base for BL99 (ppt)
         ! phi_init and dSin0_frazil are used for mushy thermo, ktherm=2
         phi_init = 0.75_real_kind, & ! initial liquid fraction of frazil
         min_salin = p1          , & ! threshold for brine pocket treatment 
         salt_loss =0.4_real_kind, &  ! fraction of salt retained in zsalinity 
         min_bgc        = 0.01_real_kind, & ! fraction of ocean bgc concentration in surface melt 
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), parameter, public :: &
         hi_ssl = 0.050_real_kind, & ! ice surface scattering layer thickness (m)
         hs_ssl = 0.040_real_kind, & ! snow surface scattering layer thickness (m)
         hs_ssl_min = 5.0e-4_real_kind ! minimum snow scattering layer thickness for aerosol accumulation (m)
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), dimension(8,31,11), public :: &  
         snowage_tau,   & ! (10^-6 m)
         snowage_kappa, & ! 
...
!-----------------------------------------------------------------------
      ...
      !-----------------------------------------------------------------
      real (kind=real_kind), dimension(max_dic), public :: &
      ...
      real (kind=real_kind), dimension(max_fe), public :: &
         fedtype 
      ...
      ! 6) dust (2.5-5 micron) 
      !-------------------------------------------------------------
      real (kind=real_kind), dimension(max_aero), public :: &
         zaerotype  
      ...
      real (kind=real_kind), dimension(max_don), public :: &  ! increase compare to algal R_Fe2C
         R_C2N_DON 
