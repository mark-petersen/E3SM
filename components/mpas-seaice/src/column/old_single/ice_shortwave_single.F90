...
! 2013, ECH: merged with NCAR version, cleaned up
      ...
      use ice_colpkg_shared_single, only: hi_ssl, hs_ssl, modal_aero, max_aero
      use ice_colpkg_shared_single, only: hi_ssl, hs_ssl, modal_aero, rsnw_fall, &
          rsnw_tmax
      ...
      real (kind=real_kind), parameter :: &
         hpmin  = 0.005_real_kind, & ! minimum allowed melt pond depth (m)
         hp0    = 0.200_real_kind    ! pond depth below which transition to bare ice
...
! Compute albedos for each thickness category
      ...
      real (kind=real_kind), parameter :: &
         dT_melt    = c1          , & ! change in temp to give dalb_mlt 
                                     ! albedo change
         dalb_mlt  = -0.075_real_kind, & ! albedo change per dT_melt change
                                     ! in temp for ice
...
! Compute albedos for each thickness category
      ...
      real (kind=real_kind), parameter :: &
         warmice  = 0.68_real_kind, &
         coldice  = 0.70_real_kind, &
         warmsnow = 0.77_real_kind, &
         coldsnow = 0.81_real_kind
...
!         C. M. Bitz, UW
      ...
      real (kind=real_kind), parameter :: &
         i0vis = 0.70_real_kind  ! fraction of penetrating solar rad (visible)
...
!         fswpenidf = swidf * (c1-alidfni) * (c1-asnow) * i0nir  
      ...
      !-----------------------------------------------------------------
         ...
         do k = 1, nilyr
            ...
            hi  = vicen / aicen
            hilyr = hi / real(nilyr,kind=real_kind)

            tranbot = exp (-kappav * hilyr * real(k,kind=real_kind))
...
! 2013 ECH merged with NCAR version
      ...
      use ice_orbital_single, only: compute_coszen
...
!echmod                     hsn = hs - hp*fpn*(c1-tmp)
         ...
         if (.not. tr_rsnw) then
           rnslyr = c1/max(c1,(real(nslyr,kind=real_kind)))
           do k = 1,nslyr
...
!
      ...
      real (kind=real_kind), dimension (4*max_aero) :: &
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), dimension (nspint) :: &
         wghtns              ! spectral weights
 
      real (kind=real_kind), parameter :: & 
         cp67    = 0.67_real_kind   , & ! nir band weight parameter
         cp33    = 0.33_real_kind   , & ! nir band weight parameter
         cp78    = 0.78_real_kind   , & ! nir band weight parameter
         cp22    = 0.22_real_kind   , & ! nir band weight parameter
         cp01    = 0.01_real_kind       ! for ocean visible albedo
 
      real (kind=real_kind), dimension (0:klev) :: &
      ...
      real (kind=real_kind), dimension (0:klevp) :: &
      ...
      real (kind=real_kind), dimension(nspint, 0:klev) :: &
      ...
      real (kind=real_kind), dimension(nmbrad), parameter :: &
         rsnw_tab = (/ &   ! snow grain radius for each table entry (micro-meters)
          5._real_kind,    7._real_kind,   10._real_kind,   15._real_kind, &
         20._real_kind,   30._real_kind,   40._real_kind,   50._real_kind, &
         65._real_kind,   80._real_kind,  100._real_kind,  120._real_kind, &
        140._real_kind,  170._real_kind,  200._real_kind,  240._real_kind, &
        290._real_kind,  350._real_kind,  420._real_kind,  500._real_kind, &
        570._real_kind,  660._real_kind,  760._real_kind,  870._real_kind, &
       1000._real_kind, 1100._real_kind, 1250._real_kind, 1400._real_kind, &
       1600._real_kind, 1800._real_kind, 2000._real_kind, 2500._real_kind/)

      ! snow extinction efficiency (unitless)
      real (kind=real_kind), dimension (nspint,nmbrad), parameter :: &
         Qs_tab = reshape((/ &
          2.131798_real_kind,  2.187756_real_kind,  2.267358_real_kind, &
          2.104499_real_kind,  2.148345_real_kind,  2.236078_real_kind, &
          2.081580_real_kind,  2.116885_real_kind,  2.175067_real_kind, &
          2.062595_real_kind,  2.088937_real_kind,  2.130242_real_kind, &
          2.051403_real_kind,  2.072422_real_kind,  2.106610_real_kind, &
          2.039223_real_kind,  2.055389_real_kind,  2.080586_real_kind, &
          2.032383_real_kind,  2.045751_real_kind,  2.066394_real_kind, &
          2.027920_real_kind,  2.039388_real_kind,  2.057224_real_kind, &
          2.023444_real_kind,  2.033137_real_kind,  2.048055_real_kind, &
          2.020412_real_kind,  2.028840_real_kind,  2.041874_real_kind, &
          2.017608_real_kind,  2.024863_real_kind,  2.036046_real_kind, &
          2.015592_real_kind,  2.022021_real_kind,  2.031954_real_kind, &
          2.014083_real_kind,  2.019887_real_kind,  2.028853_real_kind, &
          2.012368_real_kind,  2.017471_real_kind,  2.025353_real_kind, &
          2.011092_real_kind,  2.015675_real_kind,  2.022759_real_kind, &
          2.009837_real_kind,  2.013897_real_kind,  2.020168_real_kind, &
          2.008668_real_kind,  2.012252_real_kind,  2.017781_real_kind, &
          2.007627_real_kind,  2.010813_real_kind,  2.015678_real_kind, &
          2.006764_real_kind,  2.009577_real_kind,  2.013880_real_kind, &
          2.006037_real_kind,  2.008520_real_kind,  2.012382_real_kind, &
          2.005528_real_kind,  2.007807_real_kind,  2.011307_real_kind, &
          2.005025_real_kind,  2.007079_real_kind,  2.010280_real_kind, &
          2.004562_real_kind,  2.006440_real_kind,  2.009333_real_kind, &
          2.004155_real_kind,  2.005898_real_kind,  2.008523_real_kind, &
          2.003794_real_kind,  2.005379_real_kind,  2.007795_real_kind, &
          2.003555_real_kind,  2.005041_real_kind,  2.007329_real_kind, &
          2.003264_real_kind,  2.004624_real_kind,  2.006729_real_kind, &
          2.003037_real_kind,  2.004291_real_kind,  2.006230_real_kind, &
          2.002776_real_kind,  2.003929_real_kind,  2.005700_real_kind, &
          2.002590_real_kind,  2.003627_real_kind,  2.005276_real_kind, &
          2.002395_real_kind,  2.003391_real_kind,  2.004904_real_kind, &
          2.002071_real_kind,  2.002922_real_kind,  2.004241_real_kind/), &
          (/nspint,nmbrad/))

      ! snow single scattering albedo (unitless)
      real (kind=real_kind), dimension (nspint,nmbrad), parameter :: &
        ws_tab = reshape((/ &
         0.9999994_real_kind,  0.9999673_real_kind,  0.9954589_real_kind, &
         0.9999992_real_kind,  0.9999547_real_kind,  0.9938576_real_kind, &
         0.9999990_real_kind,  0.9999382_real_kind,  0.9917989_real_kind, &
         0.9999985_real_kind,  0.9999123_real_kind,  0.9889724_real_kind, &
         0.9999979_real_kind,  0.9998844_real_kind,  0.9866190_real_kind, &
         0.9999970_real_kind,  0.9998317_real_kind,  0.9823021_real_kind, &
         0.9999960_real_kind,  0.9997800_real_kind,  0.9785269_real_kind, &
         0.9999951_real_kind,  0.9997288_real_kind,  0.9751601_real_kind, &
         0.9999936_real_kind,  0.9996531_real_kind,  0.9706974_real_kind, &
         0.9999922_real_kind,  0.9995783_real_kind,  0.9667577_real_kind, &
         0.9999903_real_kind,  0.9994798_real_kind,  0.9621007_real_kind, &
         0.9999885_real_kind,  0.9993825_real_kind,  0.9579541_real_kind, &
         0.9999866_real_kind,  0.9992862_real_kind,  0.9541924_real_kind, &
         0.9999838_real_kind,  0.9991434_real_kind,  0.9490959_real_kind, &
         0.9999810_real_kind,  0.9990025_real_kind,  0.9444940_real_kind, &
         0.9999772_real_kind,  0.9988171_real_kind,  0.9389141_real_kind, &
         0.9999726_real_kind,  0.9985890_real_kind,  0.9325819_real_kind, &
         0.9999670_real_kind,  0.9983199_real_kind,  0.9256405_real_kind, &
         0.9999605_real_kind,  0.9980117_real_kind,  0.9181533_real_kind, &
         0.9999530_real_kind,  0.9976663_real_kind,  0.9101540_real_kind, &
         0.9999465_real_kind,  0.9973693_real_kind,  0.9035031_real_kind, &
         0.9999382_real_kind,  0.9969939_real_kind,  0.8953134_real_kind, &
         0.9999289_real_kind,  0.9965848_real_kind,  0.8865789_real_kind, &
         0.9999188_real_kind,  0.9961434_real_kind,  0.8773350_real_kind, &
         0.9999068_real_kind,  0.9956323_real_kind,  0.8668233_real_kind, &
         0.9998975_real_kind,  0.9952464_real_kind,  0.8589990_real_kind, &
         0.9998837_real_kind,  0.9946782_real_kind,  0.8476493_real_kind, &
         0.9998699_real_kind,  0.9941218_real_kind,  0.8367318_real_kind, &
         0.9998515_real_kind,  0.9933966_real_kind,  0.8227881_real_kind, &
         0.9998332_real_kind,  0.9926888_real_kind,  0.8095131_real_kind, &
         0.9998148_real_kind,  0.9919968_real_kind,  0.7968620_real_kind, &
         0.9997691_real_kind,  0.9903277_real_kind,  0.7677887_real_kind/), &
         (/nspint,nmbrad/))

      ! snow asymmetry parameter (unitless)
      real (kind=real_kind), dimension (nspint,nmbrad), parameter :: &
         gs_tab = reshape((/ &
          0.859913_real_kind,  0.848003_real_kind,  0.824415_real_kind, &
          0.867130_real_kind,  0.858150_real_kind,  0.848445_real_kind, &
          0.873381_real_kind,  0.867221_real_kind,  0.861714_real_kind, &
          0.878368_real_kind,  0.874879_real_kind,  0.874036_real_kind, &
          0.881462_real_kind,  0.879661_real_kind,  0.881299_real_kind, &
          0.884361_real_kind,  0.883903_real_kind,  0.890184_real_kind, &
          0.885937_real_kind,  0.886256_real_kind,  0.895393_real_kind, &
          0.886931_real_kind,  0.887769_real_kind,  0.899072_real_kind, &
          0.887894_real_kind,  0.889255_real_kind,  0.903285_real_kind, &
          0.888515_real_kind,  0.890236_real_kind,  0.906588_real_kind, &
          0.889073_real_kind,  0.891127_real_kind,  0.910152_real_kind, &
          0.889452_real_kind,  0.891750_real_kind,  0.913100_real_kind, &
          0.889730_real_kind,  0.892213_real_kind,  0.915621_real_kind, &
          0.890026_real_kind,  0.892723_real_kind,  0.918831_real_kind, &
          0.890238_real_kind,  0.893099_real_kind,  0.921540_real_kind, &
          0.890441_real_kind,  0.893474_real_kind,  0.924581_real_kind, &
          0.890618_real_kind,  0.893816_real_kind,  0.927701_real_kind, &
          0.890762_real_kind,  0.894123_real_kind,  0.930737_real_kind, &
          0.890881_real_kind,  0.894397_real_kind,  0.933568_real_kind, &
          0.890975_real_kind,  0.894645_real_kind,  0.936148_real_kind, &
          0.891035_real_kind,  0.894822_real_kind,  0.937989_real_kind, &
          0.891097_real_kind,  0.895020_real_kind,  0.939949_real_kind, &
          0.891147_real_kind,  0.895212_real_kind,  0.941727_real_kind, &
          0.891189_real_kind,  0.895399_real_kind,  0.943339_real_kind, &
          0.891225_real_kind,  0.895601_real_kind,  0.944915_real_kind, &
          0.891248_real_kind,  0.895745_real_kind,  0.945950_real_kind, &
          0.891277_real_kind,  0.895951_real_kind,  0.947288_real_kind, &
          0.891299_real_kind,  0.896142_real_kind,  0.948438_real_kind, &
          0.891323_real_kind,  0.896388_real_kind,  0.949762_real_kind, &
          0.891340_real_kind,  0.896623_real_kind,  0.950916_real_kind, &
          0.891356_real_kind,  0.896851_real_kind,  0.951945_real_kind, &
          0.891386_real_kind,  0.897399_real_kind,  0.954156_real_kind/), &
          (/nspint,nmbrad/))
      ...
      real (kind=real_kind), dimension (nspint), parameter :: &
         ki_ssl_mn = (/ 1000.1_real_kind, 1003.7_real_kind, 7042._real_kind/), &
         wi_ssl_mn = (/ .9999_real_kind,  .9963_real_kind,  .9088_real_kind/), &
         gi_ssl_mn = (/  .94_real_kind,     .94_real_kind,    .94_real_kind/)

      ! ice drained layer (dl) iops
      real (kind=real_kind), dimension (nspint), parameter :: &
         ki_dl_mn = (/ 100.2_real_kind, 107.7_real_kind,  1309._real_kind /), &
         wi_dl_mn = (/ .9980_real_kind,  .9287_real_kind, .0305_real_kind /), &
         gi_dl_mn = (/ .94_real_kind,     .94_real_kind,    .94_real_kind /)

      ! ice interior layer (int) iops
      real (kind=real_kind), dimension (nspint), parameter :: &
         ki_int_mn = (/  20.2_real_kind,  27.7_real_kind,  1445._real_kind /), &
         wi_int_mn = (/ .9901_real_kind, .7223_real_kind,  .0277_real_kind /), &
         gi_int_mn = (/ .94_real_kind,    .94_real_kind,     .94_real_kind /)

      ! ponded ice surface scattering layer (ssl) iops
      real (kind=real_kind), dimension (nspint), parameter :: &
         ki_p_ssl_mn = (/ 70.2_real_kind,  77.7_real_kind,  1309._real_kind/), &
         wi_p_ssl_mn = (/ .9972_real_kind, .9009_real_kind, .0305_real_kind/), &
         gi_p_ssl_mn = (/ .94_real_kind,   .94_real_kind,   .94_real_kind  /)

      ! ponded ice interior layer (int) iops
      real (kind=real_kind), dimension (nspint), parameter :: &
         ki_p_int_mn = (/  20.2_real_kind,  27.7_real_kind, 1445._real_kind/), &
         wi_p_int_mn = (/ .9901_real_kind, .7223_real_kind, .0277_real_kind/), &
         gi_p_int_mn = (/ .94_real_kind,   .94_real_kind,   .94_real_kind  /)
      ...
      real (kind=real_kind), dimension (nspint), parameter :: &
         kw = (/ 0.20_real_kind,   12.0_real_kind,   729._real_kind /), &
         ww = (/ 0.00_real_kind,   0.00_real_kind,   0.00_real_kind /), &
         gw = (/ 0.00_real_kind,   0.00_real_kind,   0.00_real_kind /)

      real (kind=real_kind), parameter :: &
         rhoi   = 917.0_real_kind,& ! pure ice mass density (kg/m3)
         fr_max = 1.00_real_kind, & ! snow grain adjustment factor max
         fr_min = 0.80_real_kind, & ! snow grain adjustment factor min
      ! tuning parameters
      ! ice and pond scat coeff fractional change for +- one-sigma in albedo
         fp_ice = 0.15_real_kind, & ! ice fraction of scat coeff for + stn dev in alb
         fm_ice = 0.15_real_kind, & ! ice fraction of scat coeff for - stn dev in alb
         fp_pnd = 2.00_real_kind, & ! ponded ice fraction of scat coeff for + stn dev in alb
         fm_pnd = 0.50_real_kind    ! ponded ice fraction of scat coeff for - stn dev in alb

      real (kind=real_kind),  parameter :: &   !chla-specific absorption coefficient
...
! Initialize and tune bare ice/ponded ice iops
      ...
      rnilyr = c1/real(nilyr,kind=real_kind)
      rnslyr = c1/real(nslyr,kind=real_kind)
      kii = nslyr + 1
...
!         if( hi < 1.5_real_kind ) dz_ssl = hi/30._real_kind
      dz_ssl = min(hi_ssl, hi/30._real_kind)
...
!ech: note this is highly resolution dependent!
      ...
      if (tr_bgc_N .and. dEdd_algae) then ! compute kabs_chl for chlorophyll
          ...
          enddo
            ...
            k = klev
            kabs_chl(1,k) = kalg*(0.50_real_kind/dzk(k)) 
      endif        ! kabs_chl
      ...
      do ns = 1, nspint
         ...
         if( srftyp == 0 ) then
            ...
            do k=0,nslyr
               ...
               ks = Qs*((rhosnw(ksnow)/rhoi)*3._real_kind / &
                       (4._real_kind*frsnw(ksnow)*1.0e-6_real_kind))
...
!   2013:  E Hunke merged with NCAR version
      ...
      real (kind=real_kind), dimension(0:klev), intent(in) :: &
         tau     , & ! layer extinction optical depth
         w0      , & ! layer single scattering albedo
         g           ! layer asymmetry parameter
      ...
      ! following arrays are defined at model interfaces; 0 is the top of the
      ! layer above the sea ice; klevp is the sea ice/ocean interface.
      real (kind=real_kind), dimension (0:klevp), intent(out) :: &
         trndir  , & ! solar beam down transmission from top
         trntdr  , & ! total transmission to direct beam for layers above
         trndif  , & ! diffuse transmission to diffuse beam for layers above
         rupdir  , & ! reflectivity to direct radiation for layers below
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), parameter :: &
         trmin = 0.001_real_kind   ! minimum total transmission allowed
      ...
      real (kind=real_kind), parameter :: & 
         refindx = 1.310_real_kind  , & ! refractive index of sea ice (water also)
         cp063   = 0.063_real_kind  , & ! diffuse fresnel reflectivity from above
         cp455   = 0.455_real_kind      ! diffuse fresnel reflectivity from below
      ...
      real (kind=real_kind), dimension (ngmax), parameter :: &
         gauspt     & ! gaussian angles (radians)
            = (/ .9894009_real_kind,  .9445750_real_kind, &
                 .8656312_real_kind,  .7554044_real_kind, &
                 .6178762_real_kind,  .4580168_real_kind, &
                 .2816036_real_kind,  .0950125_real_kind/), &
         gauswt     & ! gaussian weights
            = (/ .0271525_real_kind,  .0622535_real_kind, &
                 .0951585_real_kind,  .1246290_real_kind, &
                 .1495960_real_kind,  .1691565_real_kind, &
                 .1826034_real_kind,  .1894506_real_kind/)
...
!   2013:  E Hunke merged with NCAR version
      ...
      real (kind=real_kind), parameter :: &
         ...
         rsnw_fresh    =  100._real_kind, & ! freshly-fallen snow grain radius 
         rsnw_nonmelt  =  500._real_kind, & ! nonmelt snow grain radius
         rsnw_sig      =  250._real_kind    ! assumed sigma for snow grain radius
...
!-----------------------------------------------------------------------
      ...
      ! pond
      fp = 0.3_real_kind*fT*(c1-fs)
      hp = 0.3_real_kind*fT*(c1-fs)
...
! authors     Nicole Jeffery, LANL
      ...
      use ice_colpkg_tracers_single, only: nt_bgc_N, nt_zaero, tr_bgc_N, &
          tr_zaero, nlt_chl_sw, nlt_zaero_sw
      use ice_colpkg_shared_single, only: dEdd_algae, bgc_flux_type, &
           R_chl2N, min_bgc, F_abs_chl,  hi_ssl
      use ice_zbgc_shared_single, only: remap_zbgc
      ...
      real (kind=real_kind), dimension (ntrcr), intent(in) ::       &
         trcrn          ! aerosol or chlorophyll

      real (kind=real_kind), dimension (nbtrcr_sw), &
         intent(out) ::    &
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), dimension (nspint_5bd) :: &
      ...
      real (kind=real_kind), dimension (nspint_5bd) :: &  ! for ice only
      ...
      ! 5-band aersol data
      real (kind=real_kind), dimension(nspint_5bd, 0:klev) :: &
      ...
      real (kind=real_kind), parameter :: &
         sza_a0 =  0.085730_real_kind , &
         sza_a1 = -0.630883_real_kind , &
         sza_a2 =  1.303723_real_kind , &
         sza_b0 =  1.467291_real_kind , &
         sza_b1 = -3.338043_real_kind , &
         sza_b2 =  6.807489_real_kind , &
         mu_75  =  0.2588_real_kind       ! cosine of 75 degree
      ...
      ! ice surface scattering layer (ssl) iops
      real (kind=real_kind), dimension (nspint_5bd), parameter :: &
         ki_ssl_mn_5bd = (/ 1000.1_real_kind, 1003.7_real_kind, 1003.7_real_kind, &
                            7042._real_kind, 7042._real_kind /), &
         wi_ssl_mn_5bd = (/ .9999_real_kind, .9963_real_kind, .9963_real_kind, &
                            .9088_real_kind, .9088_real_kind /), &
         gi_ssl_mn_5bd = (/ .94_real_kind, .94_real_kind, .94_real_kind, &
                            .94_real_kind, .94_real_kind /)

      ! ice drained layer (dl) iops
      real (kind=real_kind), dimension (nspint_5bd), parameter :: &
         ki_dl_mn_5bd = (/ 100.2_real_kind, 107.7_real_kind, 107.7_real_kind, &
                           1309._real_kind, 1309._real_kind /), &
         wi_dl_mn_5bd = (/ .9980_real_kind, .9287_real_kind, .9287_real_kind, &
                           .0305_real_kind, .0305_real_kind /), &
         gi_dl_mn_5bd = (/ .94_real_kind, .94_real_kind, .94_real_kind, &
                           .94_real_kind, .94_real_kind /)

      ! ice interior layer (int) iops
      real (kind=real_kind), dimension (nspint_5bd), parameter :: &
         ki_int_mn_5bd = (/ 20.2_real_kind, 27.7_real_kind, 27.7_real_kind, &
                            1445._real_kind, 1445._real_kind/), &
         wi_int_mn_5bd = (/ .9901_real_kind, .7223_real_kind, .7223_real_kind, &
                            .0277_real_kind, .0277_real_kind /), &
         gi_int_mn_5bd = (/ .94_real_kind, .94_real_kind, .94_real_kind, &
                            .94_real_kind, .94_real_kind /)
...
!      wghtns(3) = cp33 + (cp22-cp33)*(c1-fnidr)
      ...
      ! add-local-variable
      wghtns_5bd_drc(1) = 1._real_kind
      wghtns_5bd_drc(2) = 0.49352158521175_real_kind!0.49352_real_kind!0.50_real_kind
      wghtns_5bd_drc(3) = 0.18099494230665_real_kind!0.18100_real_kind!0.18_real_kind
      wghtns_5bd_drc(4) = 0.12094898498813_real_kind!0.12095_real_kind!0.12_real_kind !
      wghtns_5bd_drc(5) = c1-(wghtns_5bd_drc(2)+wghtns_5bd_drc(3)+wghtns_5bd_drc(4))
      !wghtns_5bd_drc(5) = 0.20453448749347_real_kind!0.20453_real_kind!0.20_real_kind !

      ! diffuse incident
      wghtns_5bd_dfs(1) = 1._real_kind
      wghtns_5bd_dfs(2) = 0.58581507618433_real_kind!0.58582_real_kind!0.59_real_kind !
      wghtns_5bd_dfs(3) = 0.20156903770812_real_kind!0.20157_real_kind!0.20_real_kind !
      wghtns_5bd_dfs(4) = 0.10917889346386_real_kind!0.10918_real_kind!0.11_real_kind !
      wghtns_5bd_dfs(5) = c1-(wghtns_5bd_dfs(2)+wghtns_5bd_dfs(3)+wghtns_5bd_dfs(4))
      !wghtns_5bd_dfs(5) = 0.10343699264369_real_kind!0.10343_real_kind!0.10_real_kind !
...
!ech: note this is highly resolution dependent!
      ...
      if (tr_bgc_N .and. dEdd_algae) then ! compute kabs_chl for chlorophyll
          ...
          enddo
            ...
            k = klev
            kabs_chl_5bd(1,k) = kalg*(0.50_real_kind/dzk(k))
         ...
         do nsky = 1,2 ! loop for both direct beam and diffuse beam
          if (nsky == 1) then ! direc incident
            do k=0,nslyr
              ...
              elseif (ceiling(rsnw(ksnow)) - rsnw(ksnow) < 1.0e-3_real_kind) then
              ...
              ! ks = Qs*((rhosnw(ksnow)/rhoi)*3._real_kind / &
              !      (4._real_kind*rsnw(ksnow)*1.0e-6_real_kind))
