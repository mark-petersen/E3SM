...
!
      ...
      ! bio parameters for algal_dyn
       ...
       real (kind=real_kind),  dimension(max_algae), public :: &
      ...
      real (kind=real_kind), dimension(max_don), public :: & 
         R_Fe2DON       ! Fe to N of DON (nmol/umol)

      real (kind=real_kind), dimension(max_doc), public :: &  
         R_Fe2DOC       ! Fe to C of DOC (nmol/umol)

      real (kind=real_kind), parameter, public :: &
         R_gC2molC  = 12.01_real_kind ! mg/mmol C
      ...
      real (kind=real_kind), parameter, dimension(max_nbtrcr),  public :: &
         kscavz    = (/ 0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, 0.03_real_kind, 0.03_real_kind, &
                        0.03_real_kind, &
                        0.03_real_kind, 0.20_real_kind, 0.02_real_kind, &
                        0.02_real_kind, 0.01_real_kind, 0.01_real_kind, &
                        0.03_real_kind /)
      ...
      real (kind=real_kind), parameter, public :: &
         phi_sk     = 0.30_real_kind     ! skeletal layer porosity

      !-----------------------------------------------------------------
      ! general biogeochemistry
      !-----------------------------------------------------------------

      real (kind=real_kind), dimension(max_nbtrcr), public :: &
      ...
      real (kind=real_kind), parameter, public :: & 
         k_o       = 3.e-8_real_kind, & ! permeability scaling factor (m^2)
         thinS     = 0.05_real_kind     ! minimum ice thickness for brine

      real (kind=real_kind), public :: & 
         flood_frac     ! fraction of ocean/meltwater that floods  !*****

      real (kind=real_kind), parameter, public :: & 
         bphimin = 0.03_real_kind      ! minimum porosity for zbgc only
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind), parameter, public :: & 
         viscos_dynamic = 2.2_real_kind   , & ! 1.8e-3_real_kind (pure water at 0^oC) (kg/m/s)
         Dm             = 1.0e-9_real_kind, & ! molecular diffusion (m^2/s)
         Ra_c           = 0.05_real_kind      ! critical Rayleigh number for bottom convection
...
! Grids can be very different and  so can  vertical spaces.  
      ...
      real (kind=real_kind), dimension (nbyrn+3+nlyrn) :: &
           trdr        , & ! combined tracer 
           trgrid          ! combined grid 

      real (kind=real_kind), dimension (nbyrn+nlyrn+3) :: &
...
! remove tracer for very small fractional areas
      ...
      real (kind=real_kind), dimension (zlevels), intent(in) :: &
