...
! Code originally based on CSM1
      ...
      psimhu(xd) = log((c1+xd*(c2+xd))*(c1+xd*xd)/c8) &
                 - c2*atan(xd) + pih
!ech                 - c2*atan(xd) + 1.571_real_kind
      ...
      elseif (sfctype(1:3)=='ocn') then
         ...
         vmag = max(umin, wind)
         rdn  = sqrt(0.0027_real_kind/vmag &
              + .000142_real_kind + .0000764_real_kind*vmag)
      ...
      do while (abs(ustar - ustar_prev)/ustar > 0 .and. k <= natmiter)
         ...
         ! Jordan et al 1999
         psimhs = -(0.7_real_kind*hol &
                + 0.75_real_kind*(hol-14.3_real_kind) &
                * exp(-0.35_real_kind*hol) + 10.7_real_kind)
         psimh  = psimhs*stable &
...
! (2) reference temperature and humidity are NOT computed
      ...
      ! momentum flux
      !------------------------------------------------------------
         tau = rhoa * 0.0012_real_kind * wind
!AOMIP         tau = rhoa * (1.10_real_kind + c4*p01*wind) &
!AOMIP                         * wind * p001
         strx = tau * uatm
         stry = tau * vatm
      ...
      shcoef = (1.20e-3_real_kind)*cp_air*rhoa*wind
      lhcoef = (1.50e-3_real_kind)*Lheat *rhoa*wind
...
! changes: Andrew Roberts, NPS (RASM/CESM coupling and documentation)
      ...
      real (kind=real_kind), parameter :: & 
         ...
         csw       = 0.002_real_kind ,&! ice-ocn drag coefficient [0.0005,0.005]
         csa       = 0.0005_real_kind,&! ice-air drag coefficient [0.0001,0.001] 
         dragia    = 0.0012_real_kind,&! ice-air drag coefficient [0.0005,0.002] 
         ...
         Lmax      = 300._real_kind  ,&! max length of floe (m) [30,3000]
         Lmoy      = 300._real_kind  ,&! average length of floe (m) [30,1000]
         ...
         sl        = 22._real_kind   ,&! Sheltering parameter Lupkes2012 [10,30]
         lpmin     = 2.26_real_kind  ,&! min pond length (m) see Eq. 17 [1,10]
         lpmax     = 24.63_real_kind ,&! max pond length (m) see Eq. 17 [10,100]
         tanar     = p4             ,&! 0.25 sail slope = 14 deg [0.4,1]
         tanak     = p4             ,&! 0.58 keel slope = 30 deg [0.4,1]
         invsqrte  = 0.6065_real_kind,&!
         phir      = 0.8_real_kind   ,&! porosity of ridges [0.4,1]
         phik      = 0.8_real_kind   ,&! porosity of keels  [0.4,1]
         hkoverhr  = c4             ,&! hkeel/hridge ratio [4,8]
         dkoverdr  = c1             ,&! dkeel/distrdg ratio [1,5]
         sHGB      = 0.18_real_kind  ,&! Lupkes2012 Eq. 28, Hanssen1988, 
      ...
      real (kind=real_kind), parameter :: &
         ocnruf   = 0.000327_real_kind, & ! ocean surface roughness (m)
         ocnrufi  = c1/ocnruf, & ! inverse ocean roughness
         icerufi  = c1/iceruf    ! inverse ice roughness

      real (kind=real_kind), parameter :: &
         camax    = 0.02_real_kind , & ! Maximum for atmospheric drag
         cwmax    = 0.06_real_kind     ! Maximum for ocean drag
