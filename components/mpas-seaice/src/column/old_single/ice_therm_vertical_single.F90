...
!         Elizabeth C. Hunke, LANL
      ...
      real (kind=real_kind), parameter :: &
         floediam = 300.0_real_kind, & ! effective floe diameter (m)
         floeshape = 0.66_real_kind , & ! constant from Steele (unitless)
         m1 = 1.6e-6_real_kind     , & ! constant from Maykut & Perovich
                                      ! (m/s/deg^(-m2))
         m2 = 1.36_real_kind           ! constant from Maykut & Perovich
                                      ! (unitless)
      ...
      !-----------------------------------------------------------------
         ...
         if (trim(fbot_xfer_type) == 'Cdn_ocn') then
            ...
            cpchr = -cp_ocn*rhow*0.006_real_kind
         endif
   ...
   !     fbot = min (c0, frzmlt)
      ...
      !-----------------------------------------------------------------
         ...
         do n = 1, ncat
            ...
            do k = 1, nslyr
               etot = etot + qsnon(k,n) * vsnon(n)/real(nslyr,kind=real_kind)
            enddo
            
            do k = 1, nilyr
               etot = etot + qicen(k,n) * vicen(n)/real(nilyr,kind=real_kind)
            enddo                  ! nilyr
...
!         C. M. Bitz, UW
      ...
      hin    = vicen / aicen
      hsn    = vsnon / aicen
      hilyr    = hin / real(nilyr,kind=real_kind)
      hslyr    = hsn / rnslyr
...
!---!-------------------------------------------------------------------
      ...
      if (ktherm == 2) then
         do k = 1, nslyr
            if (hsn <= puny) then
               fhocnn = fhocnn &
                      + zqsn(k)*hsn/(real(nslyr,kind=real_kind)*dt)
               zqsn(k) = -rhos*Lfresh
               if (tr_snow) then
...
!         C. M. Bitz, UW
      ...
      real (kind=real_kind), dimension (nlyr) :: &
         hq              ! h * q for a layer
