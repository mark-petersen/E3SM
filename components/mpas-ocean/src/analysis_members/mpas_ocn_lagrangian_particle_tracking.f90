...
!-----------------------------------------------------------------------
   ...
   subroutine ocn_init_lagrangian_particle_tracking(domain, err)!{{{
      ...
      ! print *, 'starting ocn_init_lagrangian_particle_tracking'
      ...
      ! print *, 'finished building and allocating particles in init'
      ...
      ! print *, 'finished building and computational halos'
      ...
      ! print *, 'g_ioProcNeighs=' , g_ioProcNeighs
      ! print *, 'finished building io halos'
      ...
      ! print *, 'g_compProcNeighsNearby = ' , g_compProcNeighsNearby
      !}}}
...
!      call zero_autocorrelation_sums(domain)
      ...
      ! print *, 'finished ocn_init_lagrangian_particle_tracking'
...
!        lonVel, latVel, sumU, sumV, sumUU, sumUV, sumVV
      ...
      ! print *, 'Computing Lagrangian Particle Tracking...'
      !call mpas_timer_start("totalLPT")
      ...
      do while (associated(block)) !{{{
        ...
        if (trim(config_AM_lagrPartTrack_compute_interval) == 'dt') then
          ...
          print *, 'LIGHT output mode is not yet supported!'
        ...
        ! print *, 'beginning particle loop with particlelist associated =  ' , associated(particlelist)
...
!#endif
      ...
      ! print *, 'finished computing Lagrangian Particle Tracking'
...
!-----------------------------------------------------------------------
   subroutine ocn_restart_lagrangian_particle_tracking(domain, err)!{{{
      ...
      if (mpas_stream_mgr_ringing_alarms(domain % streamManager, streamID='lagrPartTrackRestart', &
        ...
        ! print *, 'start ocn_restart_lagrangian_particle_tracking'
        ...
        ! print *, 'end ocn_restart_lagrangian_particle_tracking'
...
!-----------------------------------------------------------------------
   subroutine write_lagrangian_particle_tracking(domain, err)!{{{
      ...
      ! print *, 'start write_lagrangian_particle_tracking'
      !call mpas_timer_start("totalLPT")
      ...
      ! print *, 'end write_lagrangian_particle_tracking'
...
!-----------------------------------------------------------------------
   subroutine ocn_finalize_lagrangian_particle_tracking(domain, err)!{{{
      ...
      ! print *, 'start ocn_finalize_lagrangian_particle_tracking'
      block => domain % blocklist
      do while (associated(block))
      ...
      ! print *, 'end ocn_finalize_lagrangian_particle_tracking'
...
!-----------------------------------------------------------------------
  subroutine compute_velocity_on_potentialdensity_surface(domain,err,aTimeLevel) !{{{
      ...
      ! print *, 'compute_velocity_on_potentialdensity_surface start'
      ...
      do while (associated(block))
        ...
        ! print *, 'nBuoyancySurfaces  =' , nBuoyancySurfaces
        ! print *, 'buoyancySurfaceValues=' , buoyancySurfaceValues
        ! print *, 'size(buoyancy)=' , size(buoyancy)
        ! print *, 'shape(buoyancy)=' , shape(buoyancy)
        zonVel = -9999.0_RKIND
        merVel = -9999.0_RKIND
      ...
      ! print *, 'compute_velocity_on_potentialdensity_surface end'
   ...
   subroutine particle_vertical_treatment(verticalTreatment, indexLevel, nCellVertices, verticesOnCell, & !{{{
     ...
     verticalTreatmentCase: select case (verticalTreatment)
       ...
       case default verticalTreatmentCase !{{{
         print *, 'Vertical treatment for particle integration unknown (' , verticalTreatment , ')!'
         return
         !}}}
...
!-----------------------------------------------------------------------
  function particle_horizontal_interpolation(nCellVertices, vertCoords, & !{{{
     ...
     ! print *, 'lambda=' , lambda
     ! print *, 'uVertex=' , uVertex(1,:)
     ! print *, 'vVertex=' , uVertex(2,:)
     ! print *, 'wVertex=' , uVertex(3,:)
...
!-----------------------------------------------------------------------
  subroutine particle_horizontal_movement(meshPool, pParticle, dpParticle) !{{{
     ...
     if(onSphere) then
       ...
       ! print *, 'dpParticle    = ' , dpParticle
       ! print *, 'pParticle     = ' , pParticle
       ! print *, 'pParticleTemp = ' , pParticleTemp
