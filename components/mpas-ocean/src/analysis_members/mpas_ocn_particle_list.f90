...
!-----------------------------------------------------------------------
    subroutine mpas_particle_list_build_halos(domain, err, namedBlock, ioProcNeighs) !{{{
      ...
      ! print *, 'sendNeigh = ' , sendNeigh
      ! print *, 'recvNeigh= ' , recvNeigh
      ...
      ! print *, 'Finished all to all with mpi_ierr = ' , mpi_ierr
      ! print *, 'sendNeigh = ' , sendNeigh
      ! print *, 'recvNeigh= ' , recvNeigh
      ! print *, 'communicate = ' , sendNeigh .or. recvNeigh
      ...
      ! print *, 'ioProcNeighs = ' , ioProcNeighs
      deallocate(tempInt)
...
!-----------------------------------------------------------------------
   subroutine mpas_particle_list_update_halos_end(domain, err, destinationName, sendProcNeighs, sendProcSendList, & !{{{
      ...
      ! print *, 'sendProcSendList = ' , sendProcSendList
      ! print *, 'sendProcRecvList = ' , sendProcRecvList
      ! print *, 'sendProcNeighs before = ' , sendProcNeighs
      ...
      do i = 1, nsendProcNeighs
        ...
        ! print *, 'sendProcNeigh= ' , sendProcNeighs(i)
        ! print *, 'send data = ' , sendProcRecvList(i,:)
      ...
      do i = 1, nsendProcNeighs
        ...
        ! print *, 'sendProcNeigh= ' , sendProcNeighs(i)
        ! print *, 'recvList before = ' , recvList(:,i)
        ! print *, 'completeList before = ' , completeList
        completeList = completeList .or. recvList(:,i)
        ! print *, 'recvList after = ' , recvList(:,i)
        ! print *, 'completeList after = ' , completeList
      end do
      ...
      ! print *, 'sendProcSendList after = ' , sendProcSendList
      ! print *, 'sendProcRecvList after = ' , sendProcRecvList
      ! print *, 'sendProcNeighs after = ' , sendProcNeighs
...
!-----------------------------------------------------------------------
   subroutine mpas_particle_list_transfer_particles_from_block_to_named_block(domain, err, & !{{{
     ...
     ! print *, 'before make_proc_to_proc_particlelists'
     call make_proc_to_proc_particlelists(domain, copyOnly, namedBlock, listPLSend, procNeighs, err)
     ! print *, 'after make_proc_to_proc_particlelists'
     ! print *, 'finished make_proc_to_proc_particlelists'
     ...
     ! print *, 'finished communicate_particle_halo_data'

     ! note that at present there is no halo data
     ! print *, 'there is no halo data to transfer currently'
     ...
     ! print *, 'finished distributing particlelist'
     ! print *, 'halo procs = ' , procNeighs
     ! print *, 'nSend = ' , nPartSend , ' nRecv = ' , nPartRecv
     ...
     ! print *, 'finished mpas_particle_list_test_num_current_particlelist'
   ...
   subroutine mpas_particle_list_write_halo_data(domain, err)!{{{
      ...
      do while (associated(block))
        ...
        call compute_ordering_vector(indexToParticleIDOriginal, indexToParticleIDNew, orderingVector)
              ! print *, 'write halo data'
              ! print *, 'indexToParticleIDOriginal =' , indexToParticleIDOriginal
              ! print *, 'indexToParticleIDNew =' , indexToParticleIDNew
              ! print *, 'ordering vector=' , orderingVector
          ...
          if (dimItr % memberType == MPAS_POOL_FIELD) then
            if (dimItr % dataType == MPAS_POOL_REAL) then
              ...
              call mpas_pool_get_field(lagrPartTrackPool, dimItr % memberName, field1DRealPointer)
                !{{{
              ! print *, 'write halo data'
              ! print *, 'member name =' , trim(dimItr % memberName)
              ! print *, 'particlelistSize= ' ,  count_particlelist(particlelist)
              ! print *, 'memory arraysize= ' ,  size(field1DRealPointer % array)
              ! print *, trim(dimItr % memberName) , ' = ' , field1DRealPointer % array
              !}}}
            ...
            elseif (dimItr % dataType == MPAS_POOL_INTEGER) then
              ...
              ! print *, "Different field type than implemented in Halo write!"
            end if
...
!             call mpas_pool_get_field(lagrPartTrackPool, dimItr % memberName, field1DRealPointer)
!               !{{{
!             ! print *, 'write nonhalo data'
!             ! print *, 'member name =' ,  dimItr % memberName
!             ! print *, 'particlelistSize= ' ,  count_particlelist(particlelist)
!             ! print *, 'memory arraysize= ' ,  size(field1DRealPointer % array)
!             !}}}
!             allocate(Array1DRealPointer(count_particlelist(particlelist)))
...
!             deallocate(Array1DIntPointer)
!           else
!             ! print *, "Different field type than implemented in nonHalo write!"
!           end if
!         elseif (dimItr % memberType == MPAS_POOL_DIMENSION) then
!           ! ignore dimensions for now and have this code so they aren't printed as an error message
!         else
...
!-----------------------------------------------------------------------
   subroutine uniqueIntegerList(array, uniqueList) !{{{
     ...
     integer, dimension(:), pointer, intent(in) :: array
      ...
      if(associated(uniqueList)) print *, 'Trying to allocate uniqueList, which is already allocated!'
...
!-----------------------------------------------------------------------
   subroutine make_proc_to_proc_particlelists(domain, copyOnly, blockSendToName, interProcPLArray, procNeighs, err) !{{{
      ...
      ! print *, 'make_proc_to_proc_particlelists'
      ...
      do while(associated(block))
        ...
        particlelist => block % particlelist
        do while(associated(particlelist))
          ! print *, 'link on particlelist ' ,  loc(particlelist)
          particle => particlelist % particle
          call mpas_pool_get_array(particle % haloDataPool, blockSendToName, particleBlock)
          ! For example:
          !call mpas_pool_get_array(particle % haloDataPool, 'currentBlock', particleBlock)

          ! print *, 'particleBlock ' ,  particleBlock ,  ' for ' ,  trim(blockSendToName)
          ! print *, 'before mpas call'
          call mpas_get_owning_proc(domain % dminfo, particleBlock, particleProc)
          ! print *, 'after mpas call'
          ! determine whether it belongs on thisBlock
          ! print *, 'myproc= ' ,  domain % dminfo % my_proc_id ,  ' particleProc= ' ,  particleProc
          ...
          if(particleProc /= domain % dminfo % my_proc_id) then
            ...
            if(arrayIndex == -1) print *, 'Found processor is not on list of "halo" processors!'
            ! print *, 'ammending particle to processor index  ' ,  arrayIndex
            ...
            if (.not.copyOnly) then
              ...
              ! 3 cases: head, middle, tail
              if(associated(particlelist % prev)) then
                ! print *, 'have previous particlelist link'
                particlelisttemp => particlelist % prev
                if (associated(particlelist % next)) then
                  ! print *, 'in middle of list'
                  ! case of the middle
                  particlelisttemp % next => particlelist % next
                  particlelisttemp2 => particlelisttemp
                  ! want to keep particle memory intact because their pointers were passed previously,
                ...
                if(associated(particlelist % next)) then
                  ...
                  ! print *, 'single link ' ,  loc(particlelist)
                  ! print *, 'block % particlelist' ,  loc(block % particlelist)
                  ! case of single link / particle
                  ! print *, 'deallocating link ' ,  loc(particlelist)
                  deallocate(particlelist)
            ...
            ! print *, 'just go to the next particle'
            particlelist => particlelist % next
          end if
        end do
...
!-----------------------------------------------------------------------
   subroutine communicate_num_particles_send_recv(domain, procNeighs, nPartSend, nPartRecv) !{{{
     ...
     ! print *, 'done with send receive calls'
     ...
     do i=1,numLists
       ...
       do while(associated(tempPL))
         ...
         if (blockNum /= block % blockID) then
           ! print *, 'block is not correct! for blockNum =' , blockNum , ' and blockID = ' , block % blockID
         end if
...
!endif
!         if (mpi_ierr /= 0) call mpas_log_write('recv: mpi_ierr = ' , mpi_ierr)
!ifdef 1
!         call MPI_WaitAll(numSends, sendRequestID, MPI_STATUSES_IGNORE, mpi_ierr)
!endif
!         if (mpi_ierr /= 0) call mpas_log_write('send: mpi_ierr = ' , mpi_ierr)
...
!         call MPI_WaitAll(numRecv, recvRequestID, MPI_STATUSES_IGNORE, mpi_ierr)
!endif
!         if (mpi_ierr /= 0) call mpas_log_write('mpi_ierr = ' , mpi_ierr)
!ifdef 1
!         call MPI_WaitAll(numSends, sendRequestID, MPI_STATUSES_IGNORE, mpi_ierr)
!endif
...
!    ! print *, 'Finished primary MPI communication for nonhalo'
...
!#endif
     ...
     ! print *, 'Finished primary MPI communication for halo'
...
!        else
!          ! print *, "Different field type than implemented during halo communication!"
!        end if
!      elseif (dimItr % memberType == MPAS_POOL_DIMENSION) then
!        ! ignore dimensions for now and have this code so they aren't printed as an error message
!      else
...
!-----------------------------------------------------------------------
   subroutine build_block_particlelists(domain, err)!{{{
      ...
      do while (associated(block))
        ...
        ! print *, 'in build_block_particlelists: nParticles = ' ,  nParticles
      ...
      do while (associated(block))
        ...
        do while(mpas_pool_get_next_member(lagrPartTrackPool, dimItr))
          ...
          if (dimItr % memberType == MPAS_POOL_FIELD) then
            if (dimItr % dataType == MPAS_POOL_REAL) then
              call mpas_pool_get_field(lagrPartTrackPool, dimItr % memberName, field1DRealPointer)
              ! print *, dimItr % memberName ,  ' = '
              ! print *, field1DRealPointer % array
            ...
            elseif (dimItr % dataType == MPAS_POOL_INTEGER) then
              ...
              if (dimItr % memberName == 'ioBlock') then
                ...
                ! print *, 'ioBlock = ' ,  field1DIntPointer % array
              end if
              ! print *, dimItr % memberName , ' = '
              ! print *, field1DIntPointer % array
              call add_halo_data_to_particle_list(particlelist, dimItr % memberName, field1DIntPointer)
            else
              ! print *, "Different field type than implemented during halo read!"
            end if
          elseif (dimItr % memberType == MPAS_POOL_DIMENSION) then
            ! ignore dimensions for now and have this code so they aren't printed as an error message
          else
...
!           else
!             ! print *, "Different field type than implemented in nonHalo read!"
!           end if
!         elseif (dimItr % memberType == MPAS_POOL_DIMENSION) then
!           ! ignore dimensions for now and have this code so they aren't printed as an error message
!         else
...
!        block => block % next
!     end do
!     ! print *, 'Finished reading non-halo data'
...
!-----------------------------------------------------------------------
   subroutine mpas_particle_list_update_halos_start(domain, block, particle, poolname, iCell, &
     ...
     ! print *, 'destinationName=' , destinationName
     ! print *, 'g_sendProcNeighs=' , gsendProcNeighs
...
!{{{
   ...
   subroutine mpas_particle_list_test_numparticles_to_neighprocs(myproc, procNeighs, ioProcNeighs) !{{{
     ...
     ! print *,  'myproc, procNeighs'
     do i=1,numNeighs
       ! print *, myproc , procNeighs(i)
     end do
     if(associated(ioProcNeighs)) then
       ! print *,  'myproc, ioProcNeighs'
       numNeighs = size(ioProcNeighs)
       do i=1,numNeighs
         ! print *, myproc , ioProcNeighs(i)
       end do
     end if
   ...
   subroutine mpas_particle_list_test_num_current_particlelist(domain) !{{{
     ...
     do while(associated(block))
       ...
       ! print *, 'block = ' , block % blockID , ' nPartList= ' ,  nPartList , ' nparticles = ' , nPartList_particles
       if (nPartList > nPartList_particles) then
         ! print *, 'Possible error! ' , nPartList - nPartList_particles , ' particles on particle list is not allocated!'
       end if
   ...
   subroutine test_currentBlock(domain) !{{{
     ...
     ! print *, 'number of particles on processor = ' , countOnProc
   ...
   subroutine test_num_particles_on_particlelist(listPL, nlist) !{{{
     ...
     do i = 1, nlist
       ...
       ! print *, 'list i=' , i , ' nparticles=' , listparticles
       sumtot = sumtot + listparticles
     end do
     ! print *, 'total_on_list=' , sumtot
