...
matplotlib.use('Agg')
"""
This script tests the sigma-z bottom following coordinate system
"""

ncfile = Dataset('vertical_grid.nc', mode='r')
nVertLevels = ncfile.dimensions['nVertLevels'].size
zBotRef = - ncfile.variables['refBottomDepth'][:] # note negative
hRef = ncfile.variables['refLayerThickness'][:]

zTopRef = np.zeros(nVertLevels+1)
zTopRef[1:nVertLevels+1] = zBotRef[:]

np.set_printoptions(precision=0)
np.set_printoptions(suppress=True)
print('nVertLevels',nVertLevels)
print('zBotRef',zBotRef)
print('zTopRef',zTopRef)
...
# determine z-level index number for this depth
    ...
    zTop = np.zeros(nVertLevels+2)
    h = np.zeros(nVertLevels+1)
    ...
    print('h',np.array_repr(h).replace('\n', '').replace(' ','').replace('array',''),'i',iCell,'D',int(-D),'S',int(sum(h)))
